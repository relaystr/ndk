...
class RelayManager {
  ...
  void startListeningToSocket(String url) {
    ...
    webSockets[url]!.asBroadcastStream(onCancel: (sub) {
      ...
      await _handleIncommingMessage(message, url);
    }, onError: (error) async {
  ...
  FutureOr<void> broadcastEvent(Nip01Event event, Iterable<String> relays, EventSigner signer) async {
  ...
  Future<Nip01Event> broadcastReaction(String eventId, String pubKey, Iterable<String> relays, EventSigner signer, {String reaction="+"}) async {
    Nip01Event event = Nip01Event(
        pubKey: pubKey,
        kind: Nip25Reaction.KIND,
        tags: [
          ["e", eventId]
        ],
        content: reaction,
  ...
  _handleIncommingMessage(dynamic message, String url) async {
    ...
    if (eventJson[0] == 'EVENT') {
      ...
      if (_subscriptions[id] != null && event.isValid && await eventVerifier.verify(event)) {
  ...
  Future<Stream<Nip01Event>> _doSubscriptionOrQuery(
      Filter filter, RelaySet relaySet,
      {bool closeOnEOSE = true, int? idleTimeout}) async {
    StreamGroup<Nip01Event> streamGroup = StreamGroup<Nip01Event>();
    List<RelayRequest> requests = relaySet.splitIntoRequests(filter);

    print(
        "request for ${filter.authors != null ? filter.authors!.length : 0} authors with kinds: ${filter.kinds} made requests to ${requests.length} relays");
  ...
  Future<void> loadMissingMetadatas(
      List<String> pubKeys, RelaySet relaySet) async {
    List<String> missingPubKeys = [];
    for (var pubKey in pubKeys) {
