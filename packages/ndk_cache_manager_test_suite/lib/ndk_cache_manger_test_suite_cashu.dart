...
void _runCashuTests(CacheManager Function() getCacheManager) {
  ...
  test('saveTransactions and getTransactions', () async {
    final cacheManager = getCacheManager();
    final transaction = CashuWalletTransaction(
      id: 'test_tx_id',
      ...
      mintUrl: 'https://tx.mint.com',
      token: 'test_token',
    );

    await cacheManager.saveTransactions(transactions: [transaction]);

    final loadedTransactions = await cacheManager.getTransactions();
    expect(loadedTransactions.length, greaterThanOrEqualTo(1));
    final found = loadedTransactions.where((t) => t.id == transaction.id);
    expect(found.length, equals(1));
    expect(found.first.changeAmount, equals(transaction.changeAmount));
  });

  test('getTransactions with filters', () async {
    final cacheManager = getCacheManager();
    final tx1 = CashuWalletTransaction(
      id: 'tx1',
      walletId: 'wallet1',
      changeAmount: 50,
      unit: 'sat',
    ...
    final tx2 = CashuWalletTransaction(
      id: 'tx2',
      walletId: 'wallet2',
      changeAmount: -25,
      unit: 'sat',
    ...
    await cacheManager.saveTransactions(transactions: [tx1, tx2]);

    final cashuTxs =
        await cacheManager.getTransactions(walletType: WalletType.CASHU);
    expect(cashuTxs.length, greaterThanOrEqualTo(2));
    expect(cashuTxs.any((t) => t.id == 'tx1'), isTrue);
  });

  test('saveWallet, getWallets, and removeWallet', () async {
    final cacheManager = getCacheManager();
    ...
    final wallet = CashuWallet(
      id: 'test_wallet_id',
      name: 'Test Wallet',
      mintUrl: 'https://wallet.mint.com',
      supportedUnits: {'sat'},
      mintInfo: mintInfo,
    );

    await cacheManager.saveWallet(wallet);
    final loadedWallets = await cacheManager.getWallets();
    expect(loadedWallets!.length, greaterThanOrEqualTo(1));
    final found = loadedWallets.where((w) => w.id == wallet.id);
    expect(found.length, equals(1));
    expect(found.first.name, equals(wallet.name));

    await cacheManager.removeWallet(wallet.id);
    final afterRemove = await cacheManager.getWallets();
    final foundAfter = afterRemove!.where((w) => w.id == wallet.id);
    expect(foundAfter.length, equals(0));
  });

  test('getWallets with ids filter', () async {
    final cacheManager = getCacheManager();
    final mintInfo = CashuMintInfo(
      name: 'Mint',
      description: 'Test',
    ...
    await cacheManager.saveWallet(wallet1);
    await cacheManager.saveWallet(wallet2);

    final filtered = await cacheManager.getWallets(ids: ['wallet1']);
    expect(filtered!.length, equals(1));
    expect(filtered[0].id, equals('wallet1'));
  });
