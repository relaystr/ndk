...
void main() async {
  group("list + metadata (external REAL)", skip: true, () {
    test('camelus - starter packs', () async {
      ...
      final recommededSets = await Future.wait(
        CAMELUS_RECOMMEDED_STARTER_PACKS.map((rPubkey) {
      ...
      final metadataFutures = allPubkeys.map(
        (pubkey) => ndk.metadata.loadMetadata(pubkey).then((metadata) {
          profiler.checkpoint('got metadata for $pubkey, ${metadata?.name}');
          return {'pubkey': pubkey, 'metadata': metadata};
        }),
      );

      final allMetadata = await Future.wait(metadataFutures);
      profiler.checkpoint('got metadata for all ${allMetadata.length} pubkeys');
