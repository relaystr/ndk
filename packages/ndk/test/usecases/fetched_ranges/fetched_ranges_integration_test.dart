...
void main() async {
  ...
  group('FetchedRanges integration', () {
    test(
      'query automatically records fetched ranges after EOSE',
      ...
      () async {
        ...
        expect(
          fetchedRanges.isNotEmpty,
          isTrue,
          reason: 'FetchedRanges should be recorded after query',
        );
        expect(
          fetchedRanges.containsKey(relay1.url),
          isTrue,
        ...
        expect(
          relayFetchedRanges.ranges.isNotEmpty,
          isTrue,
    ...
    test(
      'multiple queries merge fetched ranges',
      timeout: const Timeout(Duration(seconds: 5)),
      () async {
        MockRelay relay1 = MockRelay(
        ...
        expect(
          relayFetchedRanges.ranges.length,
          equals(1),
    ...
    test(
      'findGaps returns correct gaps after query',
      timeout: const Timeout(Duration(seconds: 5)),
      () async {
        MockRelay relay1 = MockRelay(
    ...
    test(
      'getOptimizedFilters returns filters for gaps only',
      timeout: const Timeout(Duration(seconds: 5)),
      () async {
        MockRelay relay1 = MockRelay(
