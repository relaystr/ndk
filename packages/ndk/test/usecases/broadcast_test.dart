  ...
  group('broadcast', () {
    ...
    test('broadcast respects timeout parameter', () async {
      ...
      await slowRelay.startServer(
        ...
        delayResponse: const Duration(
          seconds: 2,
        ), // Add delay to simulate slow relay
      );
      ...
      try {
        ...
        expect(
          duration,
          lessThanOrEqualTo(customTimeout + const Duration(milliseconds: 600)),
        );
      ...
      try {
        ...
        NdkBroadcastResponse response = ndk.broadcast.broadcast(
          ...
          timeout: const Duration(
            seconds: 5,
          ), // Long timeout to ensure it's not timing out
        ...
        ); // Small delay to ensure events are indexed
      } finally {
    ...
    test(
      'broadcast does not save to cache when saveToCache is false',
      () async {
        ndk.accounts.loginPrivateKey(
          pubkey: key0.publicKey,
          privkey: key0.privateKey!,
    ...
    test(
      'broadcastDeletion with event generates e and k tags (NIP-09)',
      () async {
        ndk.accounts.loginPrivateKey(
          pubkey: key0.publicKey,
          privkey: key0.privateKey!,
        ...
        final eTags = deletionEvent.tags.where(
          (t) => t.length >= 2 && t[0] == 'e',
        );
        expect(eTags.length, 1);
        expect(eTags.first[1], textNote.id);

        final kTags = deletionEvent.tags.where(
          (t) => t.length >= 2 && t[0] == 'k',
        );
    ...
    test(
      ...
      () async {
        ...
        final aTags = deletionEvent.tags.where(
          (t) => t.length >= 2 && t[0] == 'a',
        );
