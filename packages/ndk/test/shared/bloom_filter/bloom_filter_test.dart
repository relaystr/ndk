...
void main() {
  group('BloomFilter', () {
    ...
    test('throws exception with invalid probability', () {
      expect(
        () => BloomFilter(falsePositiveProbability: 0, numItems: 1000),
        throwsArgumentError,
      );
      expect(
        () => BloomFilter(falsePositiveProbability: 1, numItems: 1000),
        throwsArgumentError,
      );
      expect(
        () => BloomFilter(falsePositiveProbability: -0.1, numItems: 1000),
        throwsArgumentError,
      );
    });
    ...
    test('throws exception with invalid number of items', () {
      expect(
        () => BloomFilter(falsePositiveProbability: 0.01, numItems: 0),
        throwsArgumentError,
      );
      expect(
        () => BloomFilter(falsePositiveProbability: 0.01, numItems: -10),
        throwsArgumentError,
      );
    });
    ...
    test('serialization and deserialization', () {
      final originalFilter = BloomFilter(
        falsePositiveProbability: 0.01,
        numItems: 1000,
      );
