...
void main() {
  group('Nip51MuteEventFilter', () {
    ...
    test('should filter event with muted word in content', () {
      muteList.elements.add(
        Nip51ListElement(tag: Nip51List.kWord, value: 'secret', private: false),
      );
      filter.muteList = muteList;
    ...
    test(
      'should filter event with muted word (case-insensitive) in content',
      () {
        muteList.elements.add(
          Nip51ListElement(
            tag: Nip51List.kWord,
    ...
    test(
      'should not filter event with no t-tags even if hashtags are muted',
      () {
        muteList.elements.add(
          Nip51ListElement(
            tag: Nip51List.kHashtag,
    ...
    test('should filter based on multiple criteria (pubKey and word)', () {
      ...
      muteList.elements.add(
        Nip51ListElement(tag: Nip51List.kWord, value: 'danger', private: false),
      );
      filter.muteList = muteList;

      final eventFromMutedAuthor = createEvent(
        pubKey: 'mutedAuthor',
        content: 'safe content',
      );
      expect(
        filter.filter(eventFromMutedAuthor),
        isFalse,
        reason: "Event from muted author should be filtered",
      );

      final eventWithMutedWord = createEvent(
        pubKey: 'safeAuthor',
    ...
    test('hasMutedWord: should correctly identify muted words', () {
      muteList.elements.add(
        Nip51ListElement(tag: Nip51List.kWord, value: 'test', private: false),
      );
      muteList.elements.add(
        Nip51ListElement(tag: Nip51List.kWord, value: 'filter', private: false),
      );
      filter.muteList = muteList;
      expect(filter.hasMutedWord('this is a test message'), isTrue);
      expect(filter.hasMutedWord('apply this filter'), isTrue);
      expect(
        filter.hasMutedWord('This Is A Test Message'),
        isTrue,
      ); // case-insensitivity
    });
    ...
    test('isMutedPubKey: should correctly identify muted pubkeys', () {
      muteList.elements.add(
        Nip51ListElement(tag: Nip51List.kPubkey, value: 'pk1', private: false),
      );
      filter.muteList = muteList;
    ...
    test('hasMutedHashtag: should correctly identify muted hashtags', () {
      ...
      filter.muteList = muteList;
      final eventWithMutedTag = createEvent(
        tags: [
          ['t', 'nostr'],
        ],
      );
      final eventWithMutedTagCaps = createEvent(
        tags: [
          ['t', 'NOSTR'],
        ],
      );
    ...
    test('hasMutedHashtag: should not identify non-muted hashtags', () {
      ...
      filter.muteList = muteList;
      final eventWithoutMutedTag = createEvent(
        tags: [
          ['t', 'bitcoin'],
        ],
      );
