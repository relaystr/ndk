...
void main() {
  group('ConcurrencyLimiterMixin', () {
    ...
    test(
      'queues operations beyond the limit and runs them in FIFO order',
      () async {
    ...
    test(
      'peak in-flight never exceeds maxConcurrentRequests under load',
      () async {
    ...
    test(
      'a queued task that detects cancellation can skip its work entirely',
      () async {
    ...
    test(
      'cancelAllQueued rejects pending waiters but spares in-flight tasks',
      () async {
        final limiter = _Limiter(1);
