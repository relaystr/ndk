...
pub fn derive_secret_rust(
    ...
    // Derive path for secret: m/129372'/0'/keyset_id_int'/counter'/0
    let path_secret_str = format!(
        "m/{}'/{}'/{}'/{}'{}",
        DERIVATION_PURPOSE, DERIVATION_COIN_TYPE, keyset_id_int, counter, "/0"
    );
    let path_secret = DerivationPath::from_str(&path_secret_str)
        .map_err(|e| format!("Failed to parse secret derivation path: {}", e))?;
    
    // Derive path for blinding: m/129372'/0'/keyset_id_int'/counter'/1
    let path_blinding_str = format!(
        "m/{}'/{}'/{}'/{}'{}",
        DERIVATION_PURPOSE, DERIVATION_COIN_TYPE, keyset_id_int, counter, "/1"
    );
    let path_blinding = DerivationPath::from_str(&path_blinding_str)
        .map_err(|e| format!("Failed to parse blinding derivation path: {}", e))?;
    
    // Derive the keys by iterating through the path
    let mut derived_secret = master_key.clone();
    for child_number in path_secret.iter() {
        derived_secret = derived_secret.derive_child(child_number)
            .map_err(|e| format!("Failed to derive secret key: {}", e))?;
    }
    
    let mut derived_blinding = master_key;
    for child_number in path_blinding.iter() {
        derived_blinding = derived_blinding.derive_child(child_number)
...
mod tests {
    ...
    fn test_derive_secret() {
        ...
        let result = derive_secret(seed_phrase, passphrase, counter, keyset_id);
        assert!(result.is_ok());
        ...
        let result2 = derive_secret(seed_phrase, passphrase, 1, keyset_id).unwrap();
    ...
    fn test_derive_secret_with_passphrase() {
        let seed_phrase = "abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about";
        let result1 = derive_secret(seed_phrase, "", 0, "1a").unwrap();
        let result2 = derive_secret(seed_phrase, "test_passphrase", 0, "1a").unwrap();
    ...
    fn test_derive_secret_matches_dart_implementation() {
        ...
        for i in 0..5 {
            let result = derive_secret(mnemonic, "", i, keyset_id).unwrap();
            assert_eq!(
                result.secret_hex,
                expected_secrets[i as usize],
                "Secret mismatch for counter {}",
                i
            );
            assert_eq!(
                result.blinding_hex,
                expected_blinding_factors[i as usize],
