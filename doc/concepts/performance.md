...
To make the operation as optimized as possible we strongly recommend using `NdkEventVerifier` from the `ndk_flutter` package. It automatically selects the fastest verifier for the current platform (`WebEventVerifier` on web, `RustEventVerifier` on native).
